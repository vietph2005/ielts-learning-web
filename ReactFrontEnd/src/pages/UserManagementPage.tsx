  ...
  return (
    <MainLayout>
      <div className="max-w-5xl mx-auto py-10 px-4 min-h-[80vh]">
        ...
        <p className="text-gray-500 text-center mb-8">Manage all users in the system</p>
            ...
            <table className="min-w-full divide-y divide-emerald-200">
              <thead className="bg-emerald-100">
                <tr>
                  ...
                  <th className="px-4 py-2 text-left text-xs font-semibold text-emerald-700 uppercase">Premium</th>
              ...
              <tbody>
                {filteredUsers.map(user => (
                  <tr key={user.userName} className="border-b border-emerald-100 hover:bg-emerald-200/30 transition-colors">
                    ...
                    <td className="px-4 py-2 text-sm text-gray-800">{user.premium ? <span className="text-emerald-600 font-semibold">Yes</span> : <span className="text-red-500 font-semibold">No</span>}</td>
                    ...
                    <td className="px-4 py-2 text-sm text-gray-800 flex gap-2">
                      ...
                      {!user.premium && <Button size="sm" variant="outline" className="border-yellow-300 text-yellow-700" onClick={() => handleUpgradePremium(user.userName)}><Crown className="w-4 h-4" /></Button>}
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        {editing && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
            <div className="bg-white rounded-xl shadow-2xl border border-emerald-200 p-8 w-full max-w-md animate-fade-in-up relative">
              ...
              <h3 className="text-xl font-bold text-emerald-700 mb-4">Edit User</h3>
              <div className="mb-4">
              ...
              <div className="mb-4">
                ...
                <select value={editData.role || ""} onChange={e => setEditData(d => ({ ...d, role: e.target.value }))} className="w-full border border-emerald-200 rounded px-3 py-2 focus:border-emerald-400 focus:outline-none">
